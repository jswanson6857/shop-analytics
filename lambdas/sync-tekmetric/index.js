...
async function fetchRepairOrders(accessToken, shopId) {
  ...
  const startDateStr = startDate.toISOString(); // Full ISO 8601 format with time
  ...
  const options = {
    hostname: 'shop.tekmetric.com',
    path: `/api/v1/repair-orders?shop=${shopId}&postedDateStart=${encodeURIComponent(startDateStr)}&repairOrderStatusId=5`,
    method: 'GET',
    headers: {
